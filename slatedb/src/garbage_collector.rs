...
fn next_gc_delay(base: std::time::Duration, max: std::time::Duration, empty_streak: u32) -> std::time::Duration {
    ...
    async fn handle(&mut self, message: GcMessage) -> Result<(), SlateDBError> {
        ...
        let (resource, base, max) = match &message {
            GcMessage::Manifest => {
                ...
                ("manifest", o.interval.unwrap_or(DEFAULT_INTERVAL), o.max_interval)
            }
            GcMessage::Wal => {
                let o = self.options.wal_options.unwrap_or_default();
                ("wal", o.interval.unwrap_or(DEFAULT_INTERVAL), o.max_interval)
            }
            GcMessage::WalFence => {
                let o = self.options.wal_fence_options.unwrap_or_default();
                ("wal_fence", o.interval.unwrap_or(DEFAULT_INTERVAL), o.max_interval)
            }
            GcMessage::Compacted => {
                let o = self.options.compacted_options.unwrap_or_default();
                ("compacted", o.interval.unwrap_or(DEFAULT_INTERVAL), o.max_interval)
            }
            GcMessage::Compactions => {
                let o = self.options.compactions_options.unwrap_or_default();
                ("compactions", o.interval.unwrap_or(DEFAULT_INTERVAL), o.max_interval)
            }
            GcMessage::Detach => {
